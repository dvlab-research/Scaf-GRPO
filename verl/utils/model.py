...
def update_model_config(module_config, override_config_kwargs):
    ...
    Args:
        ...
        override_config_kwargs: The kwargs to override the module config.
    """
    print('111 module_config: ', module_config)
    print('222 override_config_kwargs: ', override_config_kwargs)
    for key, val in override_config_kwargs.items():
        if isinstance(val, dict):
            # update_model_config(getattr(module_config, key), val)

            if key == 'rope_scaling':
                if not hasattr(module_config, key) or getattr(module_config, key) is None:
                    print(f"[INFO] Initializing empty subconfig for '{key}'")
                    # init_value = {}
                    # for val_k in val.keys():
                    #     init_value[val_k] = None
                    # setattr(module_config, key, init_value)
                    sub_config = getattr(module_config, key)
                    if isinstance(sub_config, dict):
                        sub_config.update(val)
            else:
                update_model_config(getattr(module_config, key), val)
